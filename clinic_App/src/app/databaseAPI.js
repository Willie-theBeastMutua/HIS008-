...
app.post('/api/submit-form', (req, res) => {
    ...
    connection.query(alternativecontact, alternativecontactdata, (err, results) => {
        ...
        res.status(201).json({ message: 'alternative Contact submitted successfully' });
    });
    ...
    connection.query(gendersql, [formData.gender] + formData.gender, (err, results) => {
        if (err) {
            console.error('Error executing query: ' + err.stack);
    ...
    // console.log('hereeee: '+ alternativecontactID)
    ...
    connection.query('SELECT patient_NO from patient where ID_No = ' + formData.ID, (err, results) => {
        if (err) {
            console.error('Error executing query: ' + err.stack);
...
// add appointment
    connection.query(appointment, appointmentdata, (err, results) => {
        ...
        res.status(201).json({ message: 'appointment data  submitted successfully' });
    });
});
